...
public class ScanRecordActivity extends Activity implements OnClickListener,
		LoadingListener {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case FAIL:
				...
				// Toast.makeText(ScanRecordActivity.this, "FAILLURE" + result,
			...
			case SUSS:
				...
				for (int i = 0; i < listAll.size(); i++) {
					...
					adapter = new WeikeRecordAdapter(ScanRecordActivity.this,
							item2);
	...
	public void onClick(View v) {
		...
		case R.id.weke_tv_edit:
			Intent intent = new Intent(ScanRecordActivity.this,SelectWeikeActivity.class);
	...
	private void loadData() {
		...
		// Log.i("info","扫描记录="+String.format(Constant.GET_WEIKE, userId, "0"));
		httpUtils.send(HttpMethod.GET,
				String.format(Constant.GET_WEIKE, userId, "0"),
			...
			public void run() {
				...
				try {
					...
					mHandler.post(new Runnable() {
						@Override
						public void run() {
							// Toast.makeText(ScanRecordActivity.this,
							// "数据发送成功" + ",result=" + reStr,
							// Toast.LENGTH_LONG).show();
						}
					});
