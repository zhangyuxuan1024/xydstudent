...
import com.xyd.student.xydexamanalysis.utils.PayResult;
import com.xyd.student.xydexamanalysis.utils.SignUtils;
...
public class PayDemoActivity extends FragmentActivity implements
	...
	private TextView product_subject, product_describe, product_price;
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				String backresutlt = (String) msg.obj;
				PayResult payResult = new PayResult(backresutlt);
				Log.d("backresutlt", "backresutlt:" + backresutlt);
				int checksigresult = SignUtils.checkSign(backresutlt,
						RSA_PUBLIC);
				Log.d("checksigresult---------", "checkresult:"
						+ checksigresult);
				boolean checksign = false;
				if (checksigresult == SignUtils.RESULT_CHECK_SIGN_SUCCEED) {
					checksign = true;
				}
				// 支付宝返回此次支付结果及加签，建议对支付宝签名信息拿签约时支付宝提供的公钥做验签
				String resultInfo = payResult.getResult();
				boolean issucess = checkIsSucecs(resultInfo);
				String resultStatus = payResult.getResultStatus();

				// 判断resultStatus 为“9000”则代表支付成功，具体状态码代表含义可参考接口文档
				if (TextUtils.equals(resultStatus, "9000") && checksign
						&& issucess) {
	...
	private boolean checkIsSucecs(String contents) {
		...
		try {
			...
			int startindex = contents.indexOf("&success=");
			int endindex = contents.indexOf("&sign_type=");
			if (startindex == -1 || endindex == -1) {
				return revalue;
			}
			String issucess = contents.substring(startindex + prefix.length(),
					endindex);
			Log.d("checkIsSucecs", "issucess:" + issucess);
			issucess = issucess.replace("\"", "");
	...
	private void initView() {
		...
		product_describe = (TextView) this.findViewById(R.id.product_describe);
		product_price = (TextView) this.findViewById(R.id.product_price);
		titleBar = (TitleBar) this.findViewById(R.id.alipay_titlebar);

		titleBar.setLeftIcon(R.drawable.x);
		titleBar.setTitle("支付宝");
		titleBar.setTitleClickListener(this);
		product_describe.setText(describe);
		product_subject.setText(chargeName);
		product_price.setText(money + "元");
	}
	...
	private void initValues() {
		...
		money = Double.toString(amount);
		Log.i("lxw", "PayDemoActivity" + orderId + chargeName + "," + amount
				+ "," + describe);
	}
	...
	public void pay(View v) {
		...
		// 订单
		String orderInfo = getOrderInfo(chargeName, describe, money);
	...
	public String getOrderInfo(String subject, String body, String price) {
		...
		// + "\"";
		orderInfo += "&notify_url=" + "\""
				+ "http://139.129.97.230:10000/ali/inputpayorder" + "\"";
		...
		orderInfo += "&return_url=\"http://139.129.97.230:10000/ali/inputpayorder\"";
